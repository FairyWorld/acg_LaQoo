 ...
 */
fun laqooSource.decryptData(data: String, key: String, iv: String): String {
    val bytes = Base64.getDecoder().decode(data.toByteArray())
    val debytes = bytes.decrypt(key, iv)
    return debytes.decodeToString()
}

val laqooSource.preferences: SharedPreferences
    get() = laqooApplication.getInstance().preferences

/**
 * 获取默认的动漫域名
 */
fun laqooSource.getDefaultDomain(): String {
    return preferences.getString(KEY_SOURCE_DOMAIN, DEFAULT_DOMAIN) ?: DEFAULT_DOMAIN
}

suspend fun laqooSource.getDocument(url: String): Document {
...
fun openExternalPlayer(videoUrl: String) {
    val context = laqooApplication.getInstance()
    val intent = Intent(ACTION_VIEW)
    intent.addFlags(FLAG_ACTIVITY_NEW_TASK)
