...
import com.laqoo.danmaku.api.TimeBasedDanmakuSession
import com.laqoome.laqoo.data.remote.dandanplay.DandanplayDanmakuProvider.Companion.ID
import com.laqoome.laqoo.data.remote.dandanplay.dto.toDanmakuOrNull
import com.laqoome.laqoo.util.createHttpClient
...
class DandanplayDanmakuProvider @Inject constructor(
    ...
    override suspend fun fetch(
        ...
        if (!searchEpisodeResponse.success || searchEpisodeResponse.laqoos.isEmpty()) {
            return null
        }
        val firstlaqoo = searchEpisodeResponse.laqoos[0]
        val episodes = firstlaqoo.episodes
        if (episodes.isEmpty()) {
            return null
        }
