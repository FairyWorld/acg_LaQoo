...
object AnfunsSource : laqooSource {
    ...
    override suspend fun getHomeData(): List<HomeBean> {
        ...
        for ((i, el) in elements.withIndex()) {
            ...
            val homeItemBeanList = getlaqooList(el.select("div.hl-list-wrap > ul > li"))
    ...
    override suspend fun getlaqooDetail(detailUrl: String): laqooDetailBean {
        ...
        val relatedlaqoos =
            getlaqooList(document.select("div.hl-change-box1").select("li"))
    ...
    override suspend fun getWeekData(): Map<Int, List<laqooBean>> {
        ...
        elements.select("div.hl-list-wrap").forEachIndexed { index, element ->
            val dayList = getlaqooList(element.select("li"))
            weekMap[index] = dayList
        }
        return weekMap
    }

    private suspend fun getlaqooEpisodes(document: Document): Map<Int, List<EpisodeBean>> {
    ...
    private suspend fun getlaqooList(elements: Elements): List<laqooBean> {
        ...
        elements.forEach { el ->
            el.select("a").apply {
                ...
                laqooList.add(laqooBean(title, imgUrl, url, episodeName))
            }
        }
        return laqooList
    }
    ...
    override suspend fun getVideoData(episodeUrl: String): VideoBean {
        ...
        val episodes =
            getlaqooEpisodes(document.select("div.hl-tabs-box"))*/
