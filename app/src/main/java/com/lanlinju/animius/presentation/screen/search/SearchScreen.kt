...
import com.laqoome.laqoo.presentation.component.PaginationStateHandler
...
fun SearchScreen(
    ...
    val laqoosState = viewModel.laqoosState.collectAsLazyPagingItems()
    ...
    Box(Modifier.fillMaxSize()) {
        SearchBar(
            ...
            LazyVerticalGrid(
                ...
                items(count = laqoosState.itemCount) { index ->
                    val mediaFocusRequester = remember { FocusRequester() }
                    var isFocused by remember { mutableStateOf(false) }
                    val item = laqoosState[index]!!
                    MediaSmall(
                        image = item.img,
                        label = item.title,
                        onClick = {
                            onNavigateTolaqooDetail(item.detailUrl, viewModel.currentSourceMode)
                        },
                        modifier = Modifier
